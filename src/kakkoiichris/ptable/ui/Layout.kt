...
sealed interface Layout {
    ...
    class Border : Layout {
        ...
        override fun invoke(parent: Panel, components: List<Component>) {
            val top = components[TOP.toInt() - 1]
            val right = components[RIGHT.toInt() - 1]
            val bottom = components[BOTTOM.toInt() - 1]
            val left = components[LEFT.toInt() - 1]
            val center = components[CENTER.toInt() - 1]
            ...
            right.setBounds(parent.width - horizontalMaxWidth, verticalMaxHeight, horizontalMaxWidth, horizontalMaxHeight)
